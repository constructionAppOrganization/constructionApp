      ...
      db.collection("employees").onSnapshot((querySnapshot) => {
        ...
        for (let i = 0; i < array.length; i++) {
          ...
          db.collection("attendance")
            .where("employeeID", "==", array[i].key)
            .where("year", "==", year)
            .where("month", "==", month)
            .where("dayStatus", "==", "worked")
            .onSnapshot(async (querySnapshot) => {
              ...
              if (array1.length == 0) {
                ...
                window.location.replace(
                  "/adminPannel/EmployeeManager/employeeSalary"
                );
              }
              ...
              await db
                ...
                .where("designation", "==", array[i].data.designation)
        ...
        <Row style={{ margin: "20px 50px 50px 150px" }}>
          <Col>
            <Row>
              <Col md={{ span: 2, offset: 2 }}>
                {" "}
                <Button variant="outline-warning" onClick={CalculateSalary}>
                  Calculate Salary
                </Button>
              </Col>
              <Col md={{ span: 4, offset: 0 }}>
                {" "}
                <Link to="/adminPannel/EmployeeManager/salaryReport">
                  <Button
                    variant="outline-warning"
                    onClick={() => {
                      SendDetails(year, month);
                    }}
                  >
